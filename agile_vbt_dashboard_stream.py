"""Push-based live data service for the Agile VBT Streamlit dashboard."""
...
from agile_vbt_dashboard_data import LiveRecordingTail, LiveTailDelta, LiveTimedEvent
...
class DashboardBroker:
    ...
    async def start(self) -> None:
        ...
        self._watch_task = asyncio.create_task(
            self._watch_changes(),
            name="agile-vbt-dashboard-file-watcher",
        )
        self._poll_task = asyncio.create_task(
            self._poll_recordings(),
            name="agile-vbt-dashboard-file-poller",
        )
        self._heartbeat_task = asyncio.create_task(
            self._send_heartbeats(),
            name="agile-vbt-dashboard-heartbeat",
        )
    ...
    async def _watch_changes(self) -> None:
        while True:
            ...
            await self._process_changes_safely()

    async def _poll_recordings(self) -> None:
        """Catch updates that a filesystem notification may coalesce or miss."""
        while True:
            await asyncio.sleep(POLL_INTERVAL_S)
            await self._process_changes_safely()

    async def _process_changes_safely(self) -> None:
        try:

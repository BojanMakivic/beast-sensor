...
REPETITION_DATA_FILE = PROJECT_DIRECTORY / "agile_vbt_repetitions.json"
EXCEL_WORKBOOK = PROJECT_DIRECTORY / "outputs" / "agile_vbt_tracker" / "Agile VBT Workout.xlsx"
RECORDINGS_DIRECTORY = PROJECT_DIRECTORY / "outputs" / "recordings"
DASHBOARD_APP = PROJECT_DIRECTORY / "agile_vbt_dashboard_app.py"
...
def build_excel_workbook() -> None:
    ...
    title = sheet["A1"]
    title.value = "Agile VBT bar-velocity training log"
    ...
    temporary_workbook = EXCEL_WORKBOOK.with_name("Agile VBT Workout.tmp.xlsx")
...
async def stream_sensor(session: TrackerSession) -> None:
    ...
    async with BleakClient(
        ...
        if not characteristic or "notify" not in characteristic.properties:
            raise RuntimeError("The Agile VBT IMU notification characteristic is unavailable.")
        session.reset_tracker()
        print(f"Connected to Agile VBT. Excel: {EXCEL_WORKBOOK}")
...
def default_recording_path() -> Path:
    ...
    return RECORDINGS_DIRECTORY / f"agile-vbt-{stamp}.jsonl"
    ...
    try:
        while True:
            try:
                ...
                print("Agile VBT disconnected. Reconnecting...")
...
def run_dashboard(args: argparse.Namespace) -> None:
    ...
    print(
        f"Starting Agile VBT live dashboard at http://localhost:{args.port}"
    )
    print(
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Track and analyze Agile VBT bar-velocity repetitions."
    )
    parser.add_argument(
        "mode",
        nargs="?",

...
function correctedTraces(events: DashboardEvent[]): Data[] {
  ...
  for (const event of events) {
    ...
    const traceMetadata = {
      beastCorrectedEnd: x[x.length - 1],
    };
    traces.push(
      lineTrace(
        "Corrected velocity",
...
function stateRegions(samples: Sample[]): Array<Partial<Shape>> {
  ...
  const addRegion = (ended: number) => {
    for (const [xref, y0, y1] of panels) {
      shapes.push({
        type: "rect",
        x0: started,
        x1: ended,
...
function layout(samples: Sample[]): Partial<Layout> {
  const annotationLabels: Array<[number, string]> = [
    [0.75, "World-up acceleration"],
    [0.54, "Velocity"],
    [0.39, "Upward displacement"],
    [0.17, "Rest and adaptive orientation"],
    [0.0, "Adaptive sample clock"],
  ];
  return {
    height: 1050,
    autosize: true,
    margin: { l: 58, r: 58, t: 88, b: 42 },
    hovermode: "x unified",
...
export async function appendChart(
  ...
  try {
    await Plotly.relayout(chart.element, {
      shapes: chartShapes(chart.samples),
    });
  } finally {
...
async function trimCorrectedTraceHistory(
  ...
  chart.element.data.forEach((trace, index) => {
    ...
    if (
      ...
      metadata !== null &&
      "beastCorrectedEnd" in metadata &&
      Number(metadata.beastCorrectedEnd) < cutoff
    ) {
      expired.push(index);
    }
  });

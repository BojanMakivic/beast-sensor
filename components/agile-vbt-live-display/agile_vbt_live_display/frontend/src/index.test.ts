...
import AgileVbtLiveDisplay from "./index";
...
function parentElement(): HTMLElement {
  ...
  parent.innerHTML = '<div class="agile-vbt-live-root"></div>';
  document.body.append(parent);
  return parent;
}

function render(parent: HTMLElement, data: ComponentData = DATA) {
  return AgileVbtLiveDisplay(
    {
...
describe("Agile VBT live dashboard component", () => {
  beforeEach(() => {
  ...
  it("keeps one plot container while WebSocket messages update the display", async () => {
    ...
    expect(socket.url).toBe("ws://localhost:3000/api/agile-vbt/live");
    ...
    expect(
      Number(source?.compareDocumentPosition(title!) ?? 0) &
  ...
  it("reconnects automatically when a half-open socket stops receiving", () => {
    vi.useFakeTimers();
    const parent = parentElement();
    const cleanup = render(parent);
  ...
  it("coalesces queued deltas into one Plotly update", async () => {
    const parent = parentElement();
    const cleanup = render(parent);
    const socket = FakeWebSocket.instances[0];
    socket.open();
    ...
    await vi.waitFor(() => {
      expect(chartFunctions.appendChart).toHaveBeenCalledTimes(1);
    });
    cleanup?.();
  });
});

...
SCRIPT_PATH = PROJECT_DIRECTORY / "agile_vbt_sensor.py"


def _load_cli_module():
    specification = importlib.util.spec_from_file_location(
        "agile_vbt_sensor_cli",
        SCRIPT_PATH,
    )
...
class CliDefaultsTests(unittest.TestCase):
    ...
    def test_plain_live_run_records_without_diagnostic_output(self):
        with patch.object(sys, "argv", ["agile_vbt_sensor.py"]):
        ...
        self.assertTrue(arguments.record.name.startswith("agile-vbt-"))

    def test_no_record_keeps_live_recording_disabled(self):
        with patch.object(
            sys,
            "argv",
            ["agile_vbt_sensor.py", "--no-record"],
        ):
            arguments = self.cli.parse_arguments()
        self.assertEqual(arguments.mode, "live")
    ...
    def test_replay_does_not_create_a_default_recording(self):
        ...
        with patch.object(
            sys,
            "argv",
            ["agile_vbt_sensor.py", "replay", str(recording)],
        ):
            arguments = self.cli.parse_arguments()
    ...
    def test_dashboard_launcher_rejects_an_occupied_port(self):
        with patch.object(
            sys,
            "argv",
            ["agile_vbt_sensor.py", "dashboard", "--port", "8501"],
        ):
            arguments = self.cli.parse_arguments()
        with (
            patch.object(
